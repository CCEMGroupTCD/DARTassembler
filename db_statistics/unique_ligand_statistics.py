...
if __name__ == '__main__':
    ...
    for prop, configs in props.items():
        ...
        if configs['ylog']:
                plt.yscale('log')
        plt.title(f'Distribution of {prop} in unique ligands.py')
    ...
    #%% Make histogram of all elements of the unique ligands.py
    ...
    plt.yscale('log')
    plt.ylim(1)
    plt.title('Elements contained in unique ligands.py')
    ...
    #%% Make pie plot of where unique ligands.py are lost/ filtered
    uligs_numbers = {
    ...
    #%% Occurrences of unique ligands.py for which no oxidation state was given.
    plt.figure()
    sns.histplot(data=data[data['pred_charge'].isna()], x='occurrences', discrete=True)
    plt.yscale('log')
    plt.title(f'Occurrences of unique ligands.py of complexes without OS')
