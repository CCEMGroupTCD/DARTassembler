...
class JSONLinesReader:
    """
    Context manager for reading .jsonlines files, supporting both regular and bz2-compressed files.
...
def iterate_complex_db(path: Union[str, Path], molecule: str='dict', n_max=None, show_progress: bool=True) -> dict:
    raise NotImplementedError('This function is currently not used. Please implement it first.')
...
def load_full_ligand_db(path: Union[str, Path], molecule: str='dict') -> dict:
    ...
    if molecule == 'class':
        db = {name: Ligand.from_dict(mol) for name, mol in db.items()}
...
def get_correct_ligand_db_path_from_input(path) -> Path:
    """
    Returns a valid path to a ligand database file from a list of synonyms and raise errors if a path is invalid.
    """
    path = str(path)
