"""
Implements a linear equation solver based on linear fitting for calculating charges of ligands.
"""
import pandas as pd
import numpy as np
import json
...
class LinearChargeSolver:
    ...
    def calculate_scores(self, max_abs_charge: int=10):
        ...
        y_pred = deepcopy(self.df_ligands['charge'])
    ...
    def get_ligand_df(self):
        ...
        for uname, pred_charge in zip(self.unique_ligand_names, self.pred_charges):
            ...
            self.df_ligands[uname] = {
                                        ...
                                        'charge': round(pred_charge)
                                        }

            # Occurences counts how often a ligand occurs in different complexes. Multiple n_ligand_instances in a single complex are counted only once.
            # If self.use_sparse_matrix is True, self.df will not have the full occupation matrix and the n_ligand_instances cannot be calculated.
            if not self.use_sparse_matrix:
                occurences = sum(self.df[uname] != 0)
                self.df_ligands[uname]['n_ligand_instances'] = occurences
        ...
        if self.has_true_charge:
            self.df_ligands['correct'] = (self.df_ligands['charge'] == self.df_ligands[self.true_charge_name])
        ...
        try:
            ...
            self.df_ligands['charge_scale_invariant'] = self.df_ligands['scaled_pred_charge'] == self.df_ligands['charge']
        except AttributeError:
        ...
        self.df_ligands['dist_to_int'] = self.df_ligands['pred_charge_exact'] - self.df_ligands['charge']
    ...
    def get_confidence_of_prediction(self, df_ligands, expected_charge_interval=(-4, 1), cutoff=0.6) -> tuple:
        ...
        charge_is_too_high = (df_ligands['charge'] < expected_charge_interval[0]) | (df_ligands['charge'] > expected_charge_interval[1])
...
f"""N confident ligands:
    ...
    def check_standard_output(self, df_ligands):
        ...
        """
        necessary_columns = ['unique_name', 'charge', 'confidence']
    ...
    def add_ligand_charge_to_all_complexes(self, df_ligands: pd.DataFrame):
        ...
        for c_id, c in self.all_complexes.items():
            for lig in c['ligands']:
                ...
                lig['charge_LCS'] = float(lig_values.get('charge'))
    ...
    def calculate_unique_ligand_charges(self, output_uncertain_charges_as_nan: bool=False) -> pd.DataFrame:
        ...
        if output_uncertain_charges_as_nan:
            df_ligands.loc[~df_ligands['is_confident'], 'charge'] = np.nan
