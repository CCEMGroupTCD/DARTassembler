...
class GraphCreation:
    ...
    def __init__(self,
        ...
        #     else:
        #         self.pymat_JmolNN_graph(atoms=molecule, atomic_props=atomic_props)
        #
        # elif graph_creating_strategy == "molsimplifyGraphs":
        #     if atomic_props is None or atomic_props == {}:
        #         warnings.warn("Empty atomic props, no graph could be created")
        #     else:
        #         self.molsimiply_Graph(atoms=molecule, atomic_props=atomic_props)
    ...
    def convert_ase_cutoff_corrections(dict_):
        ...
        return {
            Element(an).symbol: covalent_radii[an] + correction
    ...
    #
    # def pymat_JmolNN_graph(self, atoms: Atoms, atomic_props: dict):
    #
    ...
    #
    #     labels = {i: el for i, el in enumerate(atoms.get_chemical_symbols())}
    #     for node in self.G.nodes():
    #         self.G.nodes[node]["node_label"] = labels[node]
    ...
    def get_xyz_file_format_string_from_atomic_props(atomic_props):
        ...
        is based on the method get_xyz_string
        from an BaseMolecule
        """
    ...
    # def molsimiply_Graph(self, atoms: Atoms, atomic_props: dict):
    #     """
    ...
    #
    #     # now we create the atoms.graph object
