...
class BatchAssemblerOutput(object):
...
class ComplexAssemblerOutput(object):
    ...
    def get_ligand_info_dict(self, ligands: list, max_entries: int=np.inf) -> dict:
        return {f'Ligand {i}': lig.get_csv_info(max_entries=max_entries) for i, lig in enumerate(ligands)}
