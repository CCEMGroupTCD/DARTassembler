...
"""
from DARTassembler.src.assembler.utils import assign_ligands_to_vectors
...
class IsomerFactory:
    def __init__(self, ligands: Dict[int, Ligand], target_vectors: List[Dict[Any, List[float]]], ligand_origins: List[List[float]], metal_centers, filter_duplicate_isomers: bool = True, filter_clashing_structures: bool = True,
        ...
        # Get metal_types and metal_origins from metal_centers
        metal_types = list(pd.unique([m.symbol for atoms  in metal_centers for m in atoms]))
        metal_origins = list(pd.unique([tuple(m.position) for atoms  in metal_centers for m in atoms]))
    ...
    def _validate_swap_groups(self) -> None:
        ...
        if any(swap_tag is None for swap_tag in self.swap_groups) and not all(swap_tag is None for swap_tag in self.swap_groups):
            ...
            logging.debug("No swap groups specified. Using effective ligand coordination numbers (n_eff_denticities) as swap groups.")
        ...
        logging.debug("Swap groups validated successfully.")
        return None

    def generate(self) -> List[Isomer]:
        # Generate all possible isomers by exchanging compatible ligands
        ligand_lists2 = assign_ligands_to_vectors(
            ligands=list(self.ligands),
            swap_groups=self.swap_groups
        )
        ligand_lists = ligand_lists2#[list(self.ligands)]   # todo turn ligand swapping on/off

        # Loop through each ligand set permutation and generate each isomer
        all_isomers = []
        all_ligands = []  # List to store the Ligand objects used in each isomer
        for ligand_swapped_combo in ligand_lists:
            ...
            aligned_DARTLigands = []  # list of Ligand objects used in the complex
        ...
        # ------------------------------- #
        isomers = AxialOptModifier(isomers=isomers, opt=self.opt_mono_rot).modify(target_vectors=self.target_vectors, ligand_origins=self.ligand_origins)
        ...
        # # ------------------------------- #
        isomers = IsomerClashFilter(isomers=isomers,
    ...
    def _add_metals(self, ligand_structure: Atoms) -> Atoms:
        ...
        for idx, (metal_type, metal_origin) in enumerate(zip(self.metal_types, self.metal_origins)):
            ...
            logging.debug(f"Metal [{idx + 1}/{len(self.metal_types)}]: [{metal_type}] added at origin [{metal_origin}]")
    ...
    def _gen_all_isomers(
        ...
        logging.debug(f"Generating isomers from {len(ligands)} ligand slots.")
        ...
        for ligand_atoms_list, ligand_obj_list in zip(ligands, DARTLigands):
            ...
            for idx, (combo_atoms, combo_ligands) in enumerate(zip(combinations_atoms, combinations_ligands)):
                ...
                logging.debug(f"Isomer {idx + 1}/{len(combinations_atoms)} assembled.")

        logging.debug(f"Total number of assembled isomers: {len(isomers)}.")
...
class AxialOptModifier:
    ...
    def modify(self, target_vectors, ligand_origins) -> List[Isomer]:
        ...
        for isomer in self.input_isomers:
            ...
            # mono_target_vectors = []
            # mono_ligand_origins = []
            # for target_vector, origin in zip(target_vectors, ligand_origins):
            #     if len(target_vector) == 1:
            #         mono_target_vectors.append(target_vector)
            #         mono_ligand_origins.append(origin)
            mono_target_vectors = target_vectors
            mono_ligand_origins = ligand_origins

            # Run the global optimizer.
            bounds = [[0, 360] for _ in mono_target_vectors]
            result = differential_evolution(
                self.objective_function, bounds=bounds,
                args=(target_vectors, ligand_origins, atoms),
                seed=42,  # For reproducibility
                # maxiter = 30, popsize=5, polish=False
            )
            ...
            for angle, axis, origin, tag in zip(list(result.x), target_vectors, ligand_origins, unique_tags_indices_set):
                ...
                atoms = self.rotate(atoms=atoms, vector=list(axis.values())[0], origin=origin, idc=indices, angle=angle).copy()
                logging.debug(f"Rotated ligand with tag {tag} by {angle:.2f} degrees around vector {axis} at origin {origin}.")
...
class IsomerClashFilter:
    ...
    def __init__(
        ...
        logging.debug(f"Initialized ClashFilter with {len(self.isomers)} isomers, "
    ...
    def filter(self) -> Tuple[List[Atoms], List[int]]:
        ...
        for idx, isomer in enumerate(self.isomers):
            ...
            if self.has_clash(atoms):
                ...
                logging.debug(f"Clash detected in isomer [{idx}] of [{len(self.isomers)}], tagging as 'clash'.")
            else:
                self.non_clashing_isomers.append(isomer)

        logging.debug(f"Tagged {len(self.rejected_isomers)} isomers as 'clash'.")
    ...
    def view_rejected_isomers(self):
        ...
        if not self.rejected_isomers:
            logging.debug("No rejected isomers to view.")
            return
...
class AtomsCombiner:
    ...
    def combine(self) -> Atoms:
        ...
        if self.xyz_atoms is None:
            logging.debug("No auxiliary structure found in .xyz file; returning base atoms only.")
...
class BiTransRotationModifier:
    ...
    def visualize_structures(self):
        ...
        """
        if not self.output_isomers:
            logging.debug("No output complexes found. Run opt_mono_rotation() first.")
            return
        ...
        logging.debug(f"Launching viewer for {len(structures_to_view)} structures...")
        view(structures_to_view)
