"""
This module assigns ligand geometries to ligands based on their donor atoms. It provides functions to align donor atoms to target vectors, try all geometrical isomer possibilities, and assign the geometry of a ligand.
"""
import ase
import numpy as np
import itertools
...
######### Define the target vectors for different geometries #########
x = np.array([1, 0, 0])
y = np.array([0, 1, 0])
z = np.array([0, 0, 1])
...
def assign_geometry(atoms: ase.Atoms, donor_idc: list[int]) -> tuple[str, list[ase.Atoms], list[list[int]], float, str, float]:
    ...
    try:
        ...
        raise ValueError(f'No geometry defined for n_donors {denticity}.')
...
if __name__ == '__main__':
    ...
    data = db._save_ligand_geometry_concat_xyz_files(
