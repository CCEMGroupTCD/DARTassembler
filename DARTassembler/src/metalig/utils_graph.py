...
def assert_graph_and_coordinates_are_consistent(graph: nx.Graph, atoms: list[str], graph_hash: str, donor_idc: list[int]=None, node_label='node_label'):
    ...
    # The order of the atoms in the graph is determined by the order of the nodes, which is sorted by the node index
    relabeled_atoms = [graph.nodes[i][node_label] for i in range(len(atoms))]
