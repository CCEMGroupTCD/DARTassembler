"""
This file contains functionality for quick access to atomic data. The data is mostly provided by mendeleev (but much faster than using mendeleev.element()) and in parts by pymatgen.
"""
import ast
import pandas as pd
...
ptable_dict = ptable.set_index('symbol', drop=False).to_dict(orient='index') # dict of the periodic table with element symbol as key
all_atomic_symbols = list(ptable_dict.keys()) # list of all atomic symbols
atomic_number_to_symbol = ptable.set_index('atomic_number')['symbol'].to_dict() # dict to convert atomic number to symbol
