"""
This module provides functions and classes to handle paths in the DARTassembler project.
"""
...
def projectpath(*relpaths) -> Path:
    """
    Returns an absolute path, given a relative path from the DARTassembler project origin.
    :param relpaths: relative paths to extend from the project origin as path/string or list of paths/strings
    :return: Path object pointing to the absolute path
    """
...
# This class works, but it is recommended to rather use the function `projectpath()` above because it is more concise and easier to use.
class project_path:
    def __init__(self):
