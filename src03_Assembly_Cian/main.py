...
test_batch_list = [
    {'Name': 'first test batch', 'Output_Path': '/Users/cianclarke/Documents/PhD/Complex_Assembly/CreateTMC/output_test/', 'MAX_num_complexes': '123', 'Topology_1': '[2, 1, 1, ["1", "2", "2"]]',
     'Topology_2': '[ 2, 2, [ "1", "1" ] ]',
...
test_batch_list_2 = [{'Name': 'first test batch', 'Output_Path': '/Users/cianclarke/Documents/PhD/Complex_Assembly/CreateTMC/output_test/', 'MAX_num_complexes': '100', 'Topology_1': '[3, 2, 0]',
                      'Metal_1': ['Fe', '+2', 'High'], "Isomers": "Generate Lowest Energy", "Optimisation_Choice": "False", "Random_Seed": "10"}]
...
class Assembly:
    ...
    def output_controller(self, list_of_complexes_wih_isomers, ligands, metal, metal_ox_state, output_path, metal_multiplicity, name_of_batch, complex_time_start, topology):
            ...
            if (complex_ is not None) and write_to_file:
                ...
                if isomer_counter == 0:  # If this is the first isomer of a complex then ...
                    if str(output_path)[-1] != "/":
                        ...
                        self.current_working_Directory = "RAC_" + str(''.join(choice(ascii_uppercase) for j in range(12)))  # Generate random name of complex
                ...
                if write_to_file:
                    ...
                    if isomer_counter == len(list_of_complexes_wih_isomers):  # If this is the last isomer of this complex the ...
                        for keys in Assembled_complex.ligand_props.keys():
                            info.update({"Ligand_{}".format(keys + 1): Assembled_complex.ligand_props[keys]})
    ...
    def assembly_main(self):
        ...
        F = LigandDB.from_json(json_='../data/Filtered_Jsons/filteredLigDB_Cian_Ct_ff_graph.json', type_="Ligand")  # We initiate the database in the RandomComplexAssembler
        ...
        for batch in self.list_of_batches:  # We loop through all the batches that have been received
            ...
            while self.total_assembled_complexes < MAX:  # We loop until the target number of complexes has been surpassed
                ...
                print("we are at " + str(int(batch["Random_Seed"])+i))
                self.progress_bar = round((self.total_assembled_complexes / MAX) * 100)  # May or may not use in the future
                complex_timer_start = timer()
                list_of_complexes_wih_isomers, ligands, metal, metal_ox_state, multiplicity, chosen_topology = RCA.create_random_TMC(Metals, Topologies, isomer_instruction,opt_choice)  # !!!ASSEMBLY!!!
                ...
                if list_of_complexes_wih_isomers is not None:
                    ...
                    pass
                input("Press Enter to continue...")  # Just so I can pause the program for debugging
                i += 1
