                        ...
                        if entries.is_dir() and str(entries.name) == "complexes":
                            for calculation in entries.iterdir():
                                for file in calculation.iterdir():
                                    if file.is_dir():
                                        raise ValueError
                                    elif str(file.name).endswith(".chk") and not ("_cont" in str(file.name)):
                                        counter += 1
                                        os.chdir(str(file.parent))
                                        print(f"formchk {str(file.name)} {str(file.name).replace('.chk','.fchk')}")
                                        #os.system(f"formchk {str(file.name)} {str(file.name).replace('.chk','.fchk')}")
                        else:
                            pass
