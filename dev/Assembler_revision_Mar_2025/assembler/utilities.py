    ...
    def get_and_cast(dictionary: dict, key: str, expected_type: type) -> Any:
        ...
        if not isinstance(value, expected_type):
            try:
                ...
                raise LoggedValueError(f"Fatal Error: '{key}' must be of type {expected_type.__name__}", logger)
        return value

class LigandSpec:
    REQUIRED_KEYS = {"origin", "ligand_db"}
    OPTIONAL_KEYS = {"n_max": np.inf, "temp_dent": None} # Still flagged as temporary
    ...
    def _get_vector(self, key: str, required: bool = False) -> Optional[List[float]]:
        """
        Gets a vector from the ligand data, ensuring it is a 3-element list of floats.
        ...
        :return: returns the vector as a list of floats or None if not found
        """
        key_count = sum(1 for k in self.data if k == key)
        if key_count > 1:
            raise LoggedValueError(f"Fatal Error: Ligand '{self.name}' must have exactly one '{key}' key, found {key_count}.", logger)
        value = self.data.get(key)
        if required and value is None:
            raise LoggedValueError(f"Fatal Error: '{key}' is required for ligand '{self.name}'.", logger)
        if value is not None:
            if not isinstance(value, list) or len(value) != 3:
                raise LoggedValueError(f"Fatal Error: '{key}' in ligand '{self.name}' must be a 3-element list.", logger)
...
class BatchInput:
    ...
    def __init__(self, batch: Dict[str, Any]):
        ...
        self.batch_name = HelpFunc.get_and_cast(dictionary=self.batch, key="name", expected_type=str)
        self.random_seed = HelpFunc.get_and_cast(dictionary=self.batch, key="random_seed", expected_type=int)
        self.max_num_complexes = HelpFunc.get_and_cast(dictionary=self.batch, key="max_num_complexes", expected_type=int)
        self.total_charge = HelpFunc.get_and_cast(dictionary=self.batch, key="total_charge", expected_type=int)
        self.geometry = HelpFunc.get_and_cast(dictionary=self.batch, key="geometry", expected_type=list)
    ...
    def _validate_metal(metal_data: Dict[str, Any]) -> List[float]:
        ...
        if "coord" not in metal_data or not isinstance(metal_data["coord"], list) or len(metal_data["coord"]) != 3:
            raise LoggedValueError("Fatal Error: 'metal' must contain a 'coord' key with a 3-element list", logger)
