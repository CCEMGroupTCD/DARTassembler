...
class DARTIsomer(BaseMolecule):
    ...
    @classmethod
    def from_batch_input(cls, atoms: Atoms, ligands: List[Ligand], batch_input: BatchInput, default_graph: bool = True) -> "DARTIsomer":
        """
    ...
    def _get_graph(self):
        ...
        graph = nx.Graph()
        tags = self.DART_atoms.get_array("multi_tags")
...
class IsomerFactory(object):
    ...
    def generate(self) -> List[Atoms]:
        ...
        DARTIsomers = [DARTIsomer(atoms=isomer,
                                  ligands=ligands,
                                  default_graph=True,
...
class ClashFilter:
    ...
    @classmethod
    def from_batch_input(cls, batch_input: BatchInput, isomers: List[DARTIsomer] = None):
        """
...
class ReduceIsomers:
    ...
    def __init__(self, isomers: List[DARTIsomer],
...
class AxialOptFactory:
    def __init__(self, isomers: List[DARTIsomer], opt: bool = True):
        """
        This class will take a list of ASE Atoms objects and optimize mono-coordinating ligands around their coordination axis
        """
        ...
        self.output_isomers = []
        logger.info(f"AxialOpt initialized with {len(self.input_isomers)} DARTIsomer isomers.")
    ...
    def opt_mono_rotation(self):
        ...
        for isomer in self.input_isomers:
            ...
            # Retrieve the multi_tags array
            multi_tags = atoms.get_array("multi_tags")
            unique_tags_indices_set = np.unique(multi_tags[:, 1])
            ...
            # Create new DARTIsomer from rotated structure
            new_dart = DARTIsomer(
                atoms=atoms.copy(),
            ...
            # Append the new DARTIsomer to the output complexes
            self.output_isomers.append(new_dart)

        logger.info(f"Optimized {len(self.output_isomers)} complexes with mono-coordinating ligand rotations.")
        return self.output_isomers
...
class AssemblerSave(object):
    ...
    def __init__(
        ...
        """
        self.isomers = isomers
        self.output_directory = pl.Path(output_directory).resolve()
    ...
    def from_batch_input(cls, batch_input: BatchInput, isomers: List[DARTIsomer]=None):
        """
        Create an instance of the assembler from a BatchInput object.
        :param batch_input:
        :return: AssemblerSave instance with settings from the batch input.
        """
        # Todo this class method has only some temporary inputs that need to be updated
        return cls(output_directory="test",
                   concatenate_xyz=True,
                   verbosity=2,
                   same_isomer_names=True,
                   complex_name_length=8,
                   n_max_ligands=len(batch_input.ligands),
                   isomers=isomers,
                   pre_delete=True)


    def run_batch(self, batch_settings: BatchInput) -> None:
        """
        Runs the DART assembler on a single batch.

        :param batch_settings: Dictionary with settings for one batch.
        :return: None
        """
        # Save input args for logging and tracking
        input_args = {**self.init_args, 'batch_settings': batch_settings}
        #self.gbl_outcontrol.save_settings(input_args) todo uncomment this when confident in what should be parsed

        # Set and store batch-specific settings
        self.batch_name = batch_settings.batch_name
        self.ligand_db_files = [ligand.ligand_db_path for ligand in batch_settings.ligands]
        self.n_max_complexes = batch_settings.max_num_complexes
        ...
        self.target_vectors = [ligand.vectors for ligand in batch_settings.ligands]
        self.ligand_origins = [ligand.origin for ligand in batch_settings.ligands]
        self.metal_centers = batch_settings.metals[0].metal_type
        ...
        self.batch_idx = 0  # Since this is single batch mode

        self.df_info = []
        self.assembled_complex_names = []
        self.assembled_complex_json_paths = []
        self._loaded_ligand_databases = {}

        self.n_batches = 1  # Todo this is a temporary fix, should be set to the number of batches in the batch input
    ...
    def _log_summary(self) -> None:
        ...
        # todo uncomment when multi batch saving is supported
        """ 
    ...
    def _log_batch_title_and_settings(self, batch_settings: dict):
        ...
        # todo we need to adapt this to work with the batch settings
        """
    ...
    def _save_assembled_isomer(self, isomer: DARTIsomer, isomer_idx: int):
        """
    ...
    def _add_batch_info(self, complex: DARTIsomer, success, complex_idx: int) -> None:
        """
        Add information about the batch to the batch info variable which will be saved to the batch info file.
        """
        elements = complex.DART_atoms.get_chemical_symbols()
        metal_atom_types = [s for s, t in zip(complex.DART_atoms.get_chemical_symbols(), complex.DART_atoms.get_array("multi_tags")) if t[0] == 0]
        data = {
            'success': success,
            ...
            'batch_name': self.batch_name,
            'metal_centers': metal_atom_types,
            'total_oxi_state': self.total_metal_oxidation_state,
            'charge': self.total_charge,
