...
if __name__ == "__main__":
    ...
    for batch in yaml_dict["batches"]:
        ...
        # For the sake of simplicity if we will format our inputs as lists in case we want to chang the input somewhat
        ligand_db_list = [ligand.ligand_db.get_lig_db_in_old_format() for ligand in assembly_input.ligands]
        ligand_CN_list = [ligand.effective_ligand_coordination_number for ligand in assembly_input.ligands]
        ...
        ligand_similarity_list = [i+1 for i in range(len(assembly_input.ligands))]
...
# 5. todo it is weird to me the Ligand choice generator does not yield the amount inputed by the user (max_num_assembled_complexes)
...
# 11. todo the random seed does not seem to be working
